...
if [ -d "$folder" ]; then
	first=1
	echo "Script by INDOHACKING"
fi
tarball="debian-rootfs.tar.xz"
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		echo "Dont Edit This Script."
		...
		x86)
			archurl="i386" ;;
		*)
			echo "Website xvideos.com"; exit 1 ;;
		esac
	...
	mkdir -p "$folder"
	cd "$folder"
	echo "Decompressing PUBG Script, Please Eat Your Food."
	proot --link2symlink tar -xJf ${cur}/${tarball}||:
	cd "$cur"
fi
mkdir -p debian-binds
bin=start-debian.sh
echo "Writing Launch PUBG Script"
cat > $bin <<- EOM
#!/bin/bash
cd \$(dirname \$0)
...
rm $tarball
echo "You can now launch PUBG Script with the ./${bin} script"